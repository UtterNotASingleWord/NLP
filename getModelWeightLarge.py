...
model.compile(optimizer='adam',
              ...
              metrics=['accuracy'])
model.load_weights('model/authorLarge_model.h5',by_name=True)
early_stopping=keras.callbacks.EarlyStopping(monitor='val_loss',
                              min_delta=0,
                              patience=2,
                              restore_best_weights=True,
                              verbose=0, mode='auto')
train_history=model.fit([x_trainOne,x_trainTwo],y_train, shuffle=True,batch_size=100,epochs=20,verbose=2,validation_data=[[x_testOne,x_testTwo],y_test],callbacks = [early_stopping])
...
def display_test_Sentiment(i):
    ...
    print(probility)
    predict=np.argmax(probility[0], axis=0)
    count=int(predict)
    print(count)
    print('标签label:', SentimentDict[np.argmax(y_test[i])], '预测结果:', SentimentDict[count])
    # print('标签label:', SentimentDict[y_test[i]], '预测结果:', SentimentDict[index])
