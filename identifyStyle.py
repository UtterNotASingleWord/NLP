...
from keras.layers import Bidirectional, Embedding, LSTM, Dense, Dropout, Lambda, K
...
Bidirectional_layer=Bidirectional(LSTM(64),name="Bidirectional")
...
input_one=Input(shape=(13,))
input_two=Input(shape=(13,))
DenseStyle=Dense(units=13,activation='relu',name="DenseStyle1")
...
# B=(Btext+styleB)/2
# B=np.array(B)
# Aauthor=int(A.argmax())
# Bauthor=int(B.argmax())
# LayerA=Dense(1)(Aauthor)
# LayerB=Dense(1)(Bauthor)
...
# predictionsDense = Dense(units=512, activation='relu',kernel_regularizer=l2(0.01))(merged_vector)
# predictionsDrop=Dropout(0.2)(predictionsDense)
predictions = Dense(units=1, activation='sigmoid',kernel_regularizer=l2(0.01))(merged_vector)
...
model.compile(optimizer='adam',
              ...
              metrics=['accuracy'])
train_history = model.fit([x_trainOne,x_trainTwo,train_inputOne,train_inputTwo],y_train,batch_size=32, epochs=10, verbose=2,shuffle=True, validation_split=0.01)
